...
def _notify_sensors(robot, method: str, *args) -> None:
    ...
    sensors = getattr(robot, "_sensors", None) or getattr(robot, "sensors", None) or {}
...
def record(
    ...
    if cfg.display_data:
        # Give the rerun blueprint each Paxini fingertip's name so it can build
        # one 3D panel per sensor instead of a single overlapping cloud.
